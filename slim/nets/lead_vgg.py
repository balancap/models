...
def lead_vgg_arg_scope(weight_decay=0.0005):
  ...
  Returns:
    An arg_scope.
  """
  with slim.arg_scope([slim.conv2d, conv2d_leaders, slim.fully_connected],
                      activation_fn=tf.nn.relu,
                      weights_regularizer=slim.l2_regularizer(weight_decay),
                      biases_initializer=tf.zeros_initializer):
    with slim.arg_scope([slim.conv2d, conv2d_leaders], padding='SAME') as arg_sc:
      return arg_sc


def lead_vgg_a(inputs,
               num_classes=1000,
               is_training=True,
  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d, conv2d_leaders],
      ...
      if spatial_squeeze:
        ...
        end_points[sc.name + '/fc8'] = net
      return net, end_points
lead_vgg_16.default_image_size = 224


def lead_vgg_19(inputs,
                num_classes=1000,
                is_training=True,
                dropout_keep_prob=0.5,
                spatial_squeeze=True,
  ...
  with tf.variable_scope(scope, 'vgg_19', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d, conv2d_leaders],
      ...
      if spatial_squeeze:
        ...
        end_points[sc.name + '/fc8'] = net
      return net, end_points
lead_vgg_19.default_image_size = 224

# Alias
lead_vgg_d = lead_vgg_16
lead_vgg_e = lead_vgg_19
